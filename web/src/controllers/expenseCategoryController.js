    ...
    try {
        ...
        await axios(config).then((response) => {
            ...
            if (resultado != 200) {
                ...
                res.redirect('expenseCategory/index');
                return;
            }
        });
        res.render('expenseCategory/index', { sucesso: sucesso, erro: erro, dados: dados });
    } catch (error) {
        logger.error(error);
...
exports.create = async (req, res) => {
    ...
    try {
        var config = {
            method: 'POST',
            url: `${server}:${port_api}/${version}/expenseCategory`,
            headers: {
                'Content-Type': 'application/json',
                user_id: user_id,
                company_id: company_id,
        ...
        await axios(config).then((response) => {
            ...
            if (resultado == 200 || resultado == 201) {
                req.flash('sucesso', 'Sub categoria criado com sucesso');
                req.session.save(() => res.redirect('/expenseCategory/index'));
            }
            else {
                req.flash('erro', `Ops! Ocorreu algum erro! `);
                req.session.save(() => res.redirect('back'));
...
exports.new = async (req, res) => {
    ...
    res.render('expenseCategory/new', { sucesso: sucesso, erro: erro });
}
exports.eliminate = async (req, res) => {
    ...
    try {
        var config = {
            method: 'delete',
            url: `${server}:${port_api}/${version}/expenseCategory/${id}`,
            headers: {
                'Content-Type': 'application/json',
                user_id: user_id,
                company_id: company_id,
    ...
    try {
        ...
        await axios(config).then((response) => {
            ...
            if (resultado != 200) {
                ...
                res.render('expenseCategory/edit', { erro: erro, sucesso: sucesso, dados: dados });
                return;
            }
        })
    } catch (error) {
...
exports.update = async (req, res) => {
    ...
    try {
        ...
        await axios(config).then((response) => {
            var resultado = response.data['resultado'];
            if (resultado == 200) {
                req.flash('sucesso', 'Sub categoria atualizado com sucesso');
                req.session.save(() => res.redirect('/expenseCategory/index'));
            }
            else {
                req.flash('erro', `Ops! Ocorreu algum erro! `);
